...
def attach_media_to_deal(deal_id: int, files: List[Dict]) -> List[int]:
    logger.info(f"📎 Прикрепление файлов к сделке {deal_id} (2-шаговая загрузка)")
    file_ids = []
    fields = get_deal_fields(deal_id)
    ...
    for file in files:
        ...
        try:
            ...
            get_url = f"{BITRIX_WEBHOOK}/disk.folder.uploadfile"
            init_form = {
            ...
            init_resp = requests.post(get_url, files=init_form)
